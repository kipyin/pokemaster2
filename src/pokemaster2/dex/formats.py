"""Various pokedex cli formats."""

import json as json_lib
import os

from . import resource_path
from ._graphics.cell_buffer import Buffer
from ._graphics.draw import *
...
def card(pokemon, shiny=False, mega=False):
    ...
    for mega in range(icons):
        draw_image(
            buffer,
            os.path.join(resource_path, pokemon.icon(shiny=shiny, mega=mega)),
            x0=content_width + 1 + icon_width * mega,
        )

    buffer.put_line((3, 1), pokemon.name, bg=0)
    buffer.put_line((3, 2), "%s Pokémon" % pokemon.genus.capitalize(), fg=245, bg=0)
    buffer.put_line(
        (3, 3), "%0.2f m / %0.1f kg" % (pokemon.height / 10.0, pokemon.weight / 10.0), fg=240, bg=0
    )
...
def simple(pokemon):
    print("%s (#%03d), %s Pokémon" % (pokemon.name, pokemon.number, pokemon.genus))
    print(
        "%s, %0.2f m, %0.1f kg"
        % (
            "/".join(map(lambda s: s.capitalize(), pokemon.types)),
            pokemon.height / 10.0,
            pokemon.weight / 10.0,
        )
    )
    ...
    if evolutions or ancestor:
        print(
            "Evolves %s%s%s"
            % (
                ("from %s" % ancestor) if ancestor else "",
