    ...
    for y in range(0, height, 2):
        for x in range(width):
            if x + x0 < buffer.width and y + y0 < buffer.height * 2:
                ...
                buffer.put_cell((x0 + x, (y0 + y) // 2), "▀", color_top, color_bottom)
...
def draw_type(buffer, type1, type2=None, x0=0, y0=0):
    buffer.put_line(
        (x0, y0), " %s " % type1.identifier.upper(), fg=0, bg=type_colors[type1.identifier.lower()]
    )
    if type2:
        buffer.put_line(
            (x0 + len(type1.identifier) + 3, y0),
            " %s " % type2.identifier.upper(),
            fg=0,
            bg=type_colors[type2.identifier.lower()],
        )
