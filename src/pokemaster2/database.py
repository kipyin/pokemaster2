...
#         warnings.warn(
#             "Connecting to the default database: " f"{pokedex.defaults.get_default_db_uri()}."
#         )
...
# lang_en = Language(identifier="English", id=9)


# def get_pokemon_data(id_, language=9):
#     return pokedex.db.util.get(
#         session=SESSION, table=pokedex.db.tables.Pokemon, id=id_, language=lang_en
#     )


# def _check_completeness(*args, msg="Must specify at least one value.") -> Optional[bool]:
...
#         query = query.filter(pokedex.db.tables.PokemonSpecies.identifier == species)
#     if form is not None:
#         query = query.filter(pokedex.db.tables.PokemonForm.form_identifier == form)
#     else:
...
#         growth_rate_id = pokemon.species.growth_rate_id
#         conditions = {
#             "growth_rate_id": growth_rate_id,
#         }
#     else:
#         raise ValueError("Must specify either the species or the National Pokédex ID.")
...
#     if exp is not None:
#         query = query.filter(pokedex.db.tables.Experience.experience <= exp).order_by(
#             pokedex.db.tables.Experience.experience.desc()
#         )
#     result = query.first()
#     if result is None:
...
# def get_nature(personality: int = None, identifier: str = None) -> pokedex.db.tables.Nature:
...
#         else:
#             gender = "female"
#     return SESSION.query(pokedex.db.tables.Gender).filter_by(identifier=gender).one()
...
#     return (SESSION.query(pokedex.db.tables.Move).ilter_by(**conditions)).one()
...
#         query = query.filter(pokedex.db.tables.Machine.machine_number == machine_number)
#     if move_identifier is not None:
#         query = query.filter(pokedex.db.tables.Move.identifier == move_identifier)
#     if move_id is not None:
#         query = query.filter(pokedex.db.tables.Move.id == move_id)
#     return query.one_or_none()


# def get_move_pool(species: str, move_method: str = None) -> List[pokedex.db.tables.PokemonMove]:
...
#         query = query.filter(pokedex.db.tables.PokemonMoveMethod.identifier == move_method)
#     return query.all()
